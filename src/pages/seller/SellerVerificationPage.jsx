  ...
  return <div className="max-w-2xl space-y-4">
    ...
    <form onSubmit={submit} className="space-y-3">
      <Card><CardHeader className="pb-3"><CardTitle className="text-base">১. {isHospital?"প্রতিষ্ঠানের তথ্য":"ব্যক্তিগত তথ্য"}</CardTitle></CardHeader><CardContent className="space-y-3">
        ...
        <div><Label>{isHospital?"প্রতিষ্ঠানের প্রোফাইল ছবি":"প্রোফাইল ছবি"} *</Label><ImageUploader bucket="seller-verification" folder={user.id} value={form.profile_photo_url} onUploaded={u=>updateAndDraft("profile_photo_url",u)} maxSizeKB={PROFILE_MAX}/></div>
      ...
      <Card><CardHeader className="pb-3"><CardTitle className="text-base">২. {isHospital?"ভেরিফিকেশন প্রমাণপত্র":"পেশাগত তথ্য"}</CardTitle><CardDescription>
        ...
        {isHospital ? <>
          ...
          {form.verification_type==="bmdc"&&<div className="grid gap-3 sm:grid-cols-2"><div><Label>BMDC নম্বর *</Label><Input required value={form.bmdc_registration_no} onChange={e=>updateAndDraft("bmdc_registration_no",e.target.value)}/></div><div><Label>BMDC প্রমাণপত্র</Label><ImageUploader bucket="seller-verification" folder={user.id} value={form.bmdc_document_url} onUploaded={u=>updateAndDraft("bmdc_document_url",u)} maxSizeKB={DOC_MAX}/></div></div>}
          {form.verification_type==="trade_license"&&<div className="grid gap-3 sm:grid-cols-2"><div><Label>ট্রেড লাইসেন্স নম্বর *</Label><Input required value={form.trade_license_no} onChange={e=>updateAndDraft("trade_license_no",e.target.value)}/></div><div><Label>ট্রেড লাইসেন্স কপি</Label><ImageUploader bucket="seller-verification" folder={user.id} value={form.trade_license_url} onUploaded={u=>updateAndDraft("trade_license_url",u)} maxSizeKB={DOC_MAX}/></div></div>}
          {form.verification_type==="nid"&&<div><Label>এনআইডি সামনের অংশ *</Label><ImageUploader bucket="seller-verification" folder={user.id} value={form.nid_front_url} onUploaded={u=>updateAndDraft("nid_front_url",u)} maxSizeKB={DOC_MAX} aspect="wide"/></div>}
        </> : <>
          ...
          <div><Label>BMDC প্রমাণপত্র (ঐচ্ছিক)</Label><ImageUploader bucket="seller-verification" folder={user.id} value={form.bmdc_document_url} onUploaded={u=>updateAndDraft("bmdc_document_url",u)} maxSizeKB={DOC_MAX}/></div>
          <div><Label>NID সামনের অংশ (শুধু প্রয়োজন হলে)</Label><ImageUploader bucket="seller-verification" folder={user.id} value={form.nid_front_url} onUploaded={u=>updateAndDraft("nid_front_url",u)} maxSizeKB={DOC_MAX} aspect="wide"/></div>
        </>}
