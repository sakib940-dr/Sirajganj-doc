  ...
  return (
    <Link to={productPath(product.slug)} className={cn("group block overflow-hidden rounded-xl border border-border bg-card transition-all hover:-translate-y-0.5 hover:shadow-md active:scale-[0.98]", className)}>
      ...
      <div className="space-y-1.5 p-3">
        ...
        {product.bmdc_registration_no&&<p className="line-clamp-1 text-[11px] text-muted-foreground">বিএমডিসি: {product.bmdc_registration_no}</p>}
        {chamber?.chamber_name||chamber?.shop_name ? <p className="flex items-start gap-1 text-[11px] font-medium text-foreground"><Stethoscope className="mt-0.5 h-3 w-3 shrink-0 text-primary"/><span className="line-clamp-1">{chamber.chamber_name||chamber.shop_name}</span></p> : null}
        {chamber?.address&&<p className="flex items-start gap-1 text-[11px] text-muted-foreground"><MapPin className="mt-0.5 h-3 w-3 shrink-0"/><span className="line-clamp-2">{chamber.address}</span></p>}
      </div>
    </Link>
  );
}
